...
# Escape a value for a single-quoted PHP string literal, where only backslash
# and single quote are special. Without this, a password containing a quote
# either breaks config-site.php or injects PHP into it.
php_single_quoted() {
   local s="$1"
   s="${s//\\/\\\\}"
   s="${s//\'/\\\'}"
