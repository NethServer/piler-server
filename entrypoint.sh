...
# The piler.conf keys this image drives from the environment. Listed once so the
# upstream-rename check and the conf_set calls below cannot drift apart.
PILER_CONF_KEYS=(mysqlhost mysqluser mysqldb mysqlpwd hostid tls_enable sphxhost
                 rtindex mysqlsocket pidfile)
...
# Set `key=value` in a piler.conf-style file: rewrite the line when present,
# append it when not. sed cannot add a line, and this file may come from the
# config volume or from a downstream module's template rather than from
# TMP_CONF_DIR, so a key this image drives from the environment can legitimately
# be absent. Refusing to start over that would make every consumer carry ten
# lines whose values are then overwritten.
#
# Going through one function is also what makes the escaping impossible to
# forget: an unescaped '&' in a sed replacement is replaced by the whole match,
# so the corruption looks self-consistent and reports nothing. The append path
# needs no escaping, printf writing the value literally.
conf_set() {
   ...
   # A file without a trailing newline would swallow the appended key.
...
fix_configs() {
   ...
   # Keys are matched by name, not by the placeholder value the template ships:
   # that placeholder is gone after the first start, so matching it would
   # silently stop applying the setting.
   #
   # The remaining trap is upstream renaming a key, which would leave us writing
   # one piler ignores. That is checked against the template the image itself
   # ships, which comes from the same package as the binary and is ours to
   # verify - unlike the live file, which a downstream module may supply. A
   # rename is a build problem, so fail the start rather than run misconfigured.
   local key
