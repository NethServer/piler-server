...
generated password or an unusual hostname needs no character set restriction.
...
- `piler_etc` (`/etc/piler`) — `piler.key` (see above), plus the generated
  config: `piler.conf`, `config-site.php`, `piler-nginx.conf`, `.my.cnf`,
  TLS cert/key.
  ...
  | File | On every start |
  | --- | --- |
  | `piler.conf` | the connection and daemon keys are re-templated (`mysqlhost`, `mysqlport`, `mysqluser`, `mysqlpwd`, `mysqldb`, `hostid`, `tls_enable`, `sphxhost`, `sphxport`, `rtindex`, `mysqlsocket`, `pidfile`), appended if the file does not have them; other keys are left as found |
  ...
  A file supplied from a volume or by a downstream module therefore needs to
  list only what it actually decides.
