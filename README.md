...
- `piler_etc` (`/etc/piler`) — generated config (`piler.conf`,
  `config-site.php`, `manticore.conf`, TLS cert/key) **and `piler.key`, the key
  the archived mail is encrypted with**. Back it up together with
  `piler_store`: a `piler_store` restored without the matching `piler.key`
  cannot be decrypted, and generating a new key does not recover it. Never
  delete `piler.key` to "reset" an instance — that discards access to
  ...
  effect on a restart. Hand edits to the re-templated keys do not survive; to
  regenerate a create-only file, delete it from the volume and restart —
  subject to the `piler.key` warning above.
- `piler_store` (`/var/piler/store`) — the actual archived mail. Back it up,
  together with `piler_etc`.
