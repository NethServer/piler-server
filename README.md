...
- `piler_etc` (`/etc/piler`) — `piler.key` (see above), plus the generated
  ...
  | File | On every start |
  | --- | --- |
  | `piler.conf` | the connection and daemon keys are re-templated (`mysqlhost`, `mysqluser`, `mysqlpwd`, `mysqldb`, `hostid`, `tls_enable`, `sphxhost`, `sphxport`, `rtindex`, `mysqlsocket`, `pidfile`), appended if the file does not have them; other keys are left as found |
