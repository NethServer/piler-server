...
echo "# the review findings"

# A port above 65535 would be written and then never connect, which is what the
# check exists to prevent.
for bad in 65536 70000 99999; do
   d="$(new_dir)"
...
d="$(new_dir)"
boot "$d" 'piler123' 'MANTICORE_PORT=65535'
check_file_has "65535 is still accepted" \
   "${d}/config-site.php" "'manticore:65535';"

# An empty required variable used to pass -v and fail five minutes later, in the
# database wait, instead of naming itself immediately.
for var in PILER_HOSTNAME MYSQL_HOSTNAME MYSQL_DATABASE MYSQL_USER; do
   d="$(new_dir)"
   if boot "$d" 'piler123' "${var}=" 2> /dev/null; then
      ko "an empty ${var} was accepted"
   else
      ok "an empty ${var} is rejected"
   fi
done
d="$(new_dir)"
if boot "$d" '' 2> /dev/null; then
   ko "an empty MYSQL_PASSWORD was accepted"
else
   ok "an empty MYSQL_PASSWORD is rejected"
fi

# A newline aborted a sed mid-file; a trailing one was eaten by the escapers'
# command substitution, and only in some destinations.
for ctl in 'a
b' "$(printf 'a\tb')" "$(printf 'a\rb')"; do
   d="$(new_dir)"
   if boot "$d" "$ctl" 2> /dev/null; then
      ko "a control character in MYSQL_PASSWORD was accepted"
   else
      ok "a control character in MYSQL_PASSWORD is rejected"
   fi
done
d="$(new_dir)"
boot "$d" 'pw with spaces'
...
# deployment stating RT=0 has to be overridden rather than obeyed.
d="$(new_dir)"
boot "$d" 'piler123'
sed -i "/^# generated by entrypoint/,\$d" "${d}/config-site.php"
echo "\$config['RT'] = 0;" >> "${d}/config-site.php"
boot "$d" 'piler123'
if command -v php > /dev/null; then
   check_eq "PHP sees RT=1 even when the deployment said 0" \
      "1" "$(php_value "${d}/config-site.php" RT)"
fi
check_file_has "RT is written in the enforced block" \
   "${d}/config-site.php" "\$config['RT'] = 1;"
